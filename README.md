...
cat /sys/class/hwmon/hwmon2/temp1_label  # если есть
cat /sys/class/hwmon/hwmon2/temp1_input  # температура в м°C
```
Где:

name — имя чипа/драйвера (например, coretemp, acpitz).

temp1_label — текстовое описание датчика (например, «Package id 0»).

temp1_input — температура в миллиградусах Цельсия (разделите на 1000).

Шаг 3. Сопоставьте hwmonN с выводом sensors
Найдите в выводе sensors строку с температурой, которая примерно соответствует тому, что показывает ваш виджет (например, +45.0°C).

Посмотрите, к какому чипу/драйверу она относится (например, coretemp-isa-0000).

Найдите этот чип в /sys/class/hwmon/:

```bash
grep -l "coretemp" /sys/class/hwmon/hwmon*/name
```
Вывод укажет путь вроде /sys/class/hwmon/hwmon2/name — значит, hwmon2 соответствует датчику coretemp.

Шаг 4. Уточните номер датчика (temp1, temp2 и т. д.)
